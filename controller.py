...
bi_pickle_file = "./pickles/double_word_dict.p"
...
def add_to_bigram_dictionary():
	word_dict = pickle.load(open(bi_pickle_file, 'rb'))
	word_dict = SENT.value_words_bigram(word_dict)
	pickle.dump(word_dict, open(bi_pickle_file, 'wb'))
	sorted_words = sorted(word_dict.iteritems(), key=lambda data: data[1][0])
	for word in sorted_words:
		print word
